...
DATA_PATH = BASE_DIR / "datapoints.txt"
TEST_PATH = BASE_DIR / "testpoints.txt"
...
TEST_COLOR_DEFAULT = "blue"
USER_COLOR = "red"

def load_data_from_file(filename: Path) -> list[str]:
    try:
...
def plot_data_with_test(features: list[list[float]],labels: list[int],test_features: Optional[list[list[float]]] = None,test_labels: Optional[list[int]] = None,user_point: Optional[list[float]] = None,user_label: Optional[str] = None) -> None:
    ...
    colors_train = [CLASS_COLORS.get(label, "gray") for label in labels]
    ...
    if test_features is not None:
        ...
        if test_labels:
            colors_test = [CLASS_COLORS.get(label, TEST_COLOR_DEFAULT) for label in test_labels]
        else:
            colors_test = [TEST_COLOR_DEFAULT] * len(test_features)
        plt.scatter(x_test, y_test, c=colors_test, marker='x', label="Test data", s=80)

    if user_point:
        plt.scatter(user_point[0], user_point[1], c=USER_COLOR, marker='*', s=200, label=f"User input: {user_label}")

    plt.title("Pichu or Pikachu with Test Points")
    plt.xlabel("Width (cm)")
...
def get_nearest_neighbor(test_point: list[float], features: list[list[float]], labels: list[int]) -> str:
    ...
    if not sorted_distances:
        raise ValueError("No training data available for nearest neighbor classification.")
...
def evaluate_knn_accuracy(pikachu: list[tuple[list[float], int]], pichu: list[tuple[list[float], int]], k: int, n_iterations: int, train_size: int, test_size: int) -> list[float]:
    accuracies = []
    ...
    for _ in range(n_iterations):
        ...
        train = pikachu[:train_size] + pichu[:train_size]
        test = pikachu[train_size:train_size+test_size] + pichu[train_size:train_size+test_size]
        ...
        for features, labels in zip(test_features, test_labels):
            pred = get_k_nearest_neighbors(features, train_features, train_labels, k)
            if (pred == "Pikachu" and labels == 1) or (pred == "Pichu" and labels == 0):
                correct += 1
    ...
    print(f"Mean accuracy over {n_iterations} iterations: {accuracy:.2%}")

    plt.plot(range(1, n_iterations + 1), accuracies, marker='o')
    plt.title(f"k-NN accuracy over {n_iterations} random splits")
...
def main() -> None:
    ...
    data_points = load_data_from_file(DATA_PATH)
    features, labels, skipped = parse_data(data_points)
   
    test_points = load_data_from_file(TEST_PATH)
    ...
    try:
        ...
        user_class_knn = get_k_nearest_neighbors(list(user_dimensions), features, labels, K_NEIGHBORS)
        print(f"User input (width: {user_dimensions[0]} cm, height: {user_dimensions[1]} cm):")
        print(f" - Nearest neighbor: {user_class_nn}")
        print(f" - k-NN (k={K_NEIGHBORS}): {user_class_knn}")
        print(f"Same classification: {user_class_nn == user_class_knn}")
    ...
    pikachu = [(f, l) for f, l in zip(features, labels) if l == 1]
    pichu = [(f, l) for f, l in zip(features, labels) if l == 0]
    
    evaluate_knn_accuracy(pikachu, pichu, K_NEIGHBORS, N_ITERATIONS, TRAIN_SIZE, TEST_SIZE)
